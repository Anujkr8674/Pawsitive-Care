...
function DonationForm() {
  ...
  const handleAmountChange = (event) => {
    ...
    setShowQRCode(false); // Hide QR code when amount is changed
  };
  ...
  const handleGenerateQRCode = () => {
    if (amount) {
      setShowQRCode(true); // Show QR code if amount is entered
    }
  };
        ...
        {donationType === 'monetary' && (
          <div className="monetary-donation-section">
            <div className="form-group3">
              ...
              {errors.amount && <div className="error">{errors.amount}</div>}
              <button type="button" onClick={handleGenerateQRCode}>Pay</button>
            </div>
            {showQRCode && (
              <div className="qr-code">
                <h4>Scan the QR Code to Pay</h4>
                <QRCodeSVG
                  value={`upi://pay?pa=anujnick57@okicici&pn=Donation&am=${amount}&cu=INR&tn=Donation for animal shelter`}
                />
              </div>
            )}
          </div>
